...
| `(*Event).ExtString(string) (Span, bool)` | Extension lookup by `string` key |
| `(*Event).ExtAt(int) (ExtPair, bool)` | Extension by index |
| `(*Event).All() iter.Seq2[Span, Span]` | Iterator over extensions |
...
**👉 For security policy and vulnerability reporting:** [📖 SECURITY.md](SECURITY.md)
...
4. **🍽️ The parser eats everything.** If a device sends `CEF:`, we parse it.
